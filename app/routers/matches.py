...
def get_matches(
    skip: int = 0,
    limit: int = 100,
    opponent_id: Optional[int] = Query(None, description="Filter by opponent ID"),
...
def update_match_result(match_id: int, result_update: MatchResultUpdate, db: Session = Depends(get_db)):
...
def get_opponent_statistics(db: Session = Depends(get_db)):
